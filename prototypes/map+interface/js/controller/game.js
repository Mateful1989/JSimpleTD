class GameController {
    ...
    spawnUnit() {
        console.log("spawn");
        if (this.map) {
            let spawnPoint = this.map.getSpawnPoints(0)[0];
            let destinationPoint = this.map.getDestinationPoints(0)[0];

            let path = calculateShortestPath(this.map, spawnPoint, destinationPoint);
